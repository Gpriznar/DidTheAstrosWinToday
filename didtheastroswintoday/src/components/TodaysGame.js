    ...
    axios({
      ...
      .then((response) => {
        ...
        if (games.schedule.playedStatus === "UNPLAYED") {
          ...
          games.schedule.playedStatus === "COMPLETED" ||
          games.schedule.playedStatus === "COMPLETED_PENDING_REVIEW"
        ) {
          this.setState({
            gameStatus: "FINAL",
          });
  ...
  render() {
    ...
    // game is live and updating
      ...
      body = (
        <div>
          <p>
            {this.state.awayTeam} - {this.state.awayScore}
          </p>
          <p>
            {this.state.homeTeam} - {this.state.homeScore}
          </p>
          <p>
            {this.state.topOrBottom} OF THE {this.state.inning}
          </p>
        </div>
      );
    }
    ...
    // if the game hasn't started yet, display game start time
      ...
      body = (
        <div>
          <p>
            {" "}
            {this.state.awayTeam} at {this.state.homeTeam}
          </p>
          <p> </p>
          <p> GAME STARTS AT</p>
          <p>
            ...
            {date.getMinutes().toString().padStart(2, "0")} {isAM ? "AM" : "PM"}{" "}
            (CT)
          </p>
        </div>
      );
